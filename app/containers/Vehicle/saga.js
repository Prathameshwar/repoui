 ...
 * @Last Modified time: 2018-12-26 13:07:17
 */
...
export function* callApiAddVehicle(action) {
  ...
  if (reponse.status <= 200 && _isEmpty(vehicle.file)) {
    ...
    yield putRedux(getVehicleIdAndChangeStatusDialog(clientId, true));
  } else {
...
export function* callApiUpdateVehicle(action) {
  ...
  if (reponse.status <= 200) {
    yield putRedux(routerActions.push(`${UrlPath}/vehicle-list`));
