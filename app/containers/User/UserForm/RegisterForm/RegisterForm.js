 ...
 * @Last Modified time: 2018-12-27 08:50:43
 */
...
const confirmPassword = (value, allValues) =>
  value !== allValues.password ? 'Password mismatched' : undefined;
...
class Form extends React.PureComponent {
  componentDidMount() {
    ...
    if (id !== 'register') {
      this.props.getUserInformationById(id);
    }
  }
  componentDidUpdate(prevProps) {
    const { user, location } = this.props;
    const { id } = parseQuery(location.search);
    if (prevProps.user.UserInfo !== user.UserInfo && id !== 'register') {
  ...
  resetData() {
    ...
    if (!_isEmpty(user.UserInfo.additional)) {
    ...
    if (!_isEmpty(user.UserInfo.dataRole)) {
    ...
    resetDataAttachFile(attach.filesInit || [], user.UserInfo.dataImage || []);
    resetDataRole(roles);
  }
  goToUserList() {
              ...
              <div className={classes.itemFormContent}>
                <div className={classes.contentLeft}>
                  ...
                  <Field
                    name="password"
                    type="password"
                    validate={id === 'register' ? required : noValidate}
                    component={this.renderField}
