 ...
 * @Last Modified time: 2018-12-27 08:32:52
 */
...
import { ApiGatewayUrl, UrlPath } from 'commons/constants';
...
export function* callApiGetUserInformationById(action) {
  ...
  if (!_.isUndefined(additionalInfo)) {
    ...
    yield putRedux(getNewAdditionalInformation(additional));
  }
...
export function* patchApiUserUpdate(action) {
  ...
  if (reponse.status <= 200) {
    yield putRedux(routerActions.push(`${UrlPath}/user-list`));
